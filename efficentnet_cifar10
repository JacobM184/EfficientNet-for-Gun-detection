 ...
 # dataset_sizes = {x: len(image_datasets[x]) for x in ['train', 'val']}
  ...
  class ConvNet(nn.Module):
      def __init__(self, num_classes=2):
          ...
          self.mbconv1= MBConv(32,  16,  3, 1, 1, SE, 0.2)
          #stride=2, output res=112x112
          self.mbconv2= MBConv(16,  24,  3, 2, 6, SE, 0.2)
          self.mbconv2repeat= MBConv(24,  24,  3, 1, 6, SE, 0.2)
          #stride=2, output res=56x56
          self.mbconv3= MBConv(24,  40,  5, 2, 6, SE, 0.2)
          self.mbconv3repeat= MBConv(40,  40,  5, 1, 6, SE, 0.2)
          #stride=2, output res=28x28
          self.mbconv4= MBConv(40,  80,  3, 2, 6, SE, 0.2)
          self.mbconv4repeat= MBConv(80,  80,  3, 1, 6, SE, 0.2)
          #stride=1, output res=28x28
          self.mbconv5= MBConv(80,  112, 5, 1, 6, SE, 0.2)
          self.mbconv5repeat= MBConv(112,  112, 5, 1, 6, SE, 0.2)
          #stride=2, output res=14x14
          self.mbconv6= MBConv(112, 192, 5, 2, 6,SE, 0.2)
          self.mbconv6repeat= MBConv(192, 192, 5, 1, 6, SE, 0.2)
          #stride=2, output res=7x7
          self.mbconv7= MBConv(192, 320, 3, 1, 6,SE, 0.2)
  ...
  # Train the model
  checkpoint = torch.load('april24.pt')
